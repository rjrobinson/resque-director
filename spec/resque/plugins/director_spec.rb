...
describe Resque::Plugins::Director do
  ...
  describe "#after_enqueue_scale_workers" do
    ...
    it "should set the queue" do
      Resque.enqueue(TestJob)
      TestJob.instance_variable_get(:@config).queue.should == "test"
    end

    it "should not set the queue if it was manually set" do
      TestJob.direct :queue => "real_queue"
      Resque.enqueue(TestJob)
      TestJob.instance_variable_get(:@config).queue.should == "real_queue"
    end
  ...
  describe "#after_pop_direct_workers" do
    describe "with time" do
      ...
      it "should set the queue if not set" do
        ...
        TestJob.instance_variable_get(:@config).queue = nil
